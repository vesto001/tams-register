...
public class UserDaoImpl implements UserDao {
	...
	public boolean insertUser(User user) {
		...
		String sql="insert into users "
				+ "(firstname, lastname, username, contact, password, email, batchid, statusid, roleid, createdBy,createdDate)"
				+ "values(?,?,?,?,?,?,?,?,?,?,?)";
		jdbcTemplate.update(sql, new Object[] {
				user.getFirstName(),user.getLastName(),user.getUserName(),user.getContact(),user.getPassword(),
				user.getEmail(), user.getBatchId(), user.getStatusId(), user.getRoleId(), user.getCreatedBy(), user.getCreatedDate()});
